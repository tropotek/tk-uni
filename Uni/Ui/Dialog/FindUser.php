...
class FindUser extends Iface
{
    ...
    public function show()
    {
        ...
        $this->addCss('dialog-find-user');
        $this->setAttr('data-ajax-url', \Uni\Uri::create('/ajax/user/findFiltered.html'));
        $this->setAttr('data-action-url', \Uni\Uri::create()->set($this->getSelectButtonId()));
        $this->setAttr('data-filter', json_encode($this->filter));
...
jQuery(function($) {
  ...
  $('.dialog-find-user').each(function () {
    var dialog = $(this);
    var settings = dialog.data();
    
    processing(false);
    dialog.find('.btn-search').click(function(e) {
      processing(true);
      var params = settings.filter;
      params.keywords = dialog.find('.input-search').val(); 
      $.get(settings.ajaxUrl, params, function (data) {
    ...
    function buildTable(data) {
      ...
      var table = $('<table class="table" style="margin-top: 10px;">' +
        '<tr><th>Name</th><th>Email</th><th>UID</th></tr>' +
        '<tr class="data-tpl"><td class="name"><a href="javascript:;" class="nameUrl"></a></td><td class="email"></td><td class="uid"></td></tr>' +
        '</table>'
      );
      $.each(data, function (i, user) {
        ...
        row.find('.name .nameUrl').text(user.name).attr('href', settings.actionUrl+'&userHash='+user.hash);
  ...
  <div class="col-md-12">
    <div class="input-group has-feedback has-feedback-left">
      ...
      <span class="input-group-btn">
        <button type="button" class="btn btn-default btn-sm btn-search" var="btn-search">Go!</button>
      </span>
    </div><!-- /input-group -->
  </div>
